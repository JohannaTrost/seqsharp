...
def joint_log_lk(pro_w, cl_w=None, profs=None, data=None, p_sites_profs=None):
    ...
    per MSA cluster (profile weights)
    :param cl_w: (n_clusters) MSA cluster probability (cluster weights)
    if None then there is either only one cluster or iEM parameters and emp_pdfs
...
def select_init_msas(emp_freq_pcs, n_runs, n_clusters, strat='circle'):
    ...
    of average AA frequencies of MSAs (1 emp_pdfs point = 1 MSA), to represent the
    ...
    if strat == 'kmeans':
        ...
        if n_clusters > 1:
            # circle includes about 80% of the emp_pdfs
...
def get_dips(lks):
    ...
    :param lks: (n_runs, n_iters) (lower bound) loglks of emp_pdfs
